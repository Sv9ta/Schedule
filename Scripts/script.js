        ...
        function WritePriv() {
            ...
            for (z = 0; z < $('.mon').length; z++) {
                if (mon[z].visibility != "hidden" && RM)
                    $('.mon:eq(' + z + ')').append('<div class="div-mantu">9:00-15:00 R-манту<div>');
            }

            var tur = $('.tur');
            for (z = 0; z < $('.tur').length; z++) {
                if (tur[z].visibility != "hidden" && Oth)
                    $('.tur:eq(' + z + ')').append('<div class="div-other">9:00-15:00 другая вакцина<div>');
            }

            var wen = $('.wen');
            for (z = 0; z < $('.wen').length; z++) {
                if (wen[z].visibility != "hidden" && Oth)
                    $('.wen:eq(' + z + ')').append('<div class="div-other">9:00-15:00 другая вакцина<div>');
            }

            var fry = $('.fry');
            for (z = 0; z < $('.fry').length; z++) {
                if (fry[z].visibility != "hidden" && Oth)
                    $('.fry:eq(' + z + ')').append('<div class="div-other">9:00-15:00 другая вакцина<div>');
            }
            ...
            for (z = 0; z < $('.fir').length; z++) {
                if (fir[z].visibility != "hidden") {
                    num++;
                    if ((num == 1 || num == 3) && BCZ)
                        $('.fir:eq(' + z + ')').append('<div class="div-bcz">10:00-11:00 БЦЖ<div>');
                    if ((num == 2 || num == 4 || num == 5) && Oth)
                        $('.fir:eq(' + z + ')').append('<div class="div-other">9:00-14:00 другая вакцина<div>');
                }
            }
        }
